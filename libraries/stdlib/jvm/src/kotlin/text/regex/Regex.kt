...
internal constructor(private val nativePattern: Pattern) : Serializable {
    ...
    public actual fun findAt(input: CharSequence, index: Int): MatchResult? =
        nativePattern.matcher(input).region(index, input.length).run {
            return if (lookingAt()) MatcherMatchResult(this, input) else null
        }

    public actual fun containsMatchAt(input: CharSequence, index: Int): Boolean =
        nativePattern.matcher(input).region(index, input.length).lookingAt()
