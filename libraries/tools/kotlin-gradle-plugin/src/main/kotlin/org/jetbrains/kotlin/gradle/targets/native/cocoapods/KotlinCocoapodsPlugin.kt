...
open class KotlinCocoapodsPlugin : Plugin<Project> {
    ...
    private fun createSyncTask(
        ...
        val platforms = project.findProperty(PLATFORM_PROPERTY)?.toString()?.split(",", " ")?.filter { it.isNotBlank() } ?: return@whenEvaluated
        val archs = project.findProperty(ARCHS_PROPERTY)?.toString()?.split(",", " ")?.filter { it.isNotBlank() } ?: return@whenEvaluated
